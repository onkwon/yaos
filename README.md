...
커널 스택은 늘 top에서 시작. 시스템 콜과 같은 핸들러를 선점가능 상태로 만들면 유저스택으로 전환되기 때문에 해당 코드에서는 스택 오버플로우를 유념, 동적할당을 이용할 것.

커널(특권)모드에서는 커널 스택이 사용되는 반면 유저(일반)모드에서는 유저(태스크) 스택이 사용된다. 특권모드는 인터럽트를 포함한다. 따라서 인터럽트 핸들러 역시 커널 스택을 사용한다. 시스템 콜과 스케줄링(문맥전환)은 태스크 실행 상태에서만 발생한다. 인터럽트 > 시스템 콜 > 일반(커널/유저) 태스크

커널 스택이든 유저 스택이든 기본적인 레지스터들은 사용중이던 스택에 일차적으로 저장되는데, 커널 스택을 공유할 경우를 고려하여 그외의 태스크 문맥은 각 유저스택에 저장하는 것을 기본으로 한다. 따라서 커널 태스크는 유저 스택을 사용할 일이 없지만, 초기 유저 스택 포인터는 커널 스택 포인터와 일치시켜야 한다.
...
권환은 커널모드와 유저모드 두가지 종류로 구분할 수 있다. 커널모드가 특권모드라면 유저모드는 일반모드이다. 커널모드는 커널 태스크와 인터럽트 핸들러 두가지 경우로 다시 분리할 수 있는데, 커널 태스크는 선점 가능한 반면 인터럽트 핸들러는 선점이 불가능하다. 즉, 스케줄링은 최하위 우선순위 인터럽트로써 동작하고 중첩되지 않는다. 이는 커널 구조의 복잡도와 데드락과 같은 논리적 오류를 최소화하기 위한 결정이다. 이 방식에서 시스템의 안정성과 응답성을 유지하기 위해서는 시스템 콜과 같은 핸들러 루틴에서의 연산을 최소화해야 한다. 블럭 디바이스 접근과 같은 막대한 지연을 유발하는 `read()` 연산과 같은 경우 핸들러 진입시 ~~태스크를 일시적으로 커널 태스크(스레드)로 상향조정한 뒤 스케줄링`yield()`하면 해당 태스크는 슬립 가능한 일반 커널 태스크로서 스케줄링된다. 그리고 모든 연산이 종료된 핸들러 탈출 시점에서 기존의 유저 권환으로 복귀한다.~~ current를 waiting 시키고 커널 태스크를 fork해서 해당 연산 수행. 시스템 콜은 그대로 종료하고 바로 스케줄링을 요청하면 current는 제어권을 잃는다. 커널 태스크가 해당 연산을 종료한 시점에 잠든 current를 깨우고 리턴값을 넘겨주면 done. 리턴값을 넘기는 부분이 문젠데, 스택을 조작해야 하나? 선점가능한 커널 태스크로 전환됨에 따라 해당 코드는 재진입가능하도록 작성되어야 하며, 동기화 문제 역시 고려되어야 한다. 일반 인터럽트의 경우 핸들러는 최소한의 주요 연산만 수행하고 나머지 연산은 `softirq`에 등록해 선점 가능한 태스크로 전환해 처리하도록 한다.

스택은 모드에 따라 커널스택, 유저스택 두가지 스택이 존재한다. 유저스택은 유저모드에서 사용되며, 커널스택은 커널모드(커널 태스크 및 인터럽트 핸들러)에서 사용된다.
