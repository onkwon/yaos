...
ARM 아키텍처 ABI(AAPCS) 은 스택을 8 바이트 단위로 정렬할 것을 권고한다. ABI 는 외부 인터페이스를 위한 프로토콜이므로 다른 함수를 호출하지 않는 말단 함수(leaf function)의 경우 해당 규약으로부터 자유롭다고 생각할 수도 있겠으나, 말단 함수에서 정렬이 깨진 상태로 인터럽트나 예외가 발생할 경우 문제가 생길 수 있음을 주지해야 한다.
