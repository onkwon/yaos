...
static inline void busfault(void)
{
	if (SCB_CFSR & IBUSERR)
...
static inline void usagefault(void)
{
...
void __attribute__((naked)) ISR_fault(void)
{
	dsb();
	hw_context_save(current);
	...
	printk("\nKernel Space\n");
	debug_print_kernel_status((unsigned int *)sp, lr, psr);

	printk("\nUser Space\n");
	debug_print_user_status((unsigned int *)usp);

	printk("\nTask Status\n");
	debug_print_task_status(current);

	printk("\nCurrent Context\n");
	debug_print_context((unsigned int *)current->stack.p);

	if (IS_FROM_THREAD(lr) && current != &init_task) {
#ifdef CONFIG_FPU
